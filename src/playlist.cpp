...
const std::vector<TrackEntry>& Playlist::Items() const { return items_; }
int Playlist::CurrentIndex() const { return current_; }
int Playlist::SelectedIndex() const { return selected_; }
...
int Playlist::NextIndex(RepeatMode repeat, bool shuffle) const {
    ...
    if (shuffle) return RandomIndex(current_);
...
int Playlist::FindIndexByPath(const std::filesystem::path& path) const {
    for (size_t i = 0; i < items_.size(); i++) {
        if (items_[i].path == path) return (int)i;
    }
    return -1;
}
