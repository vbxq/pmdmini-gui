...
void Player::SetVolume(int volume) { volume_.store(volume); }
void Player::SetMute(bool mute) { mute_.store(mute); }

void Player::SetOutputDevice(const std::string& name) {
    auto next = (name == "Default") ? "" : name;
    if (next == output_device_) return;
...
PlayerState Player::GetState() const { return state_.load(); }
bool Player::IsLoading() const { return loading_.load(); }
std::string Player::GetOutputDevice() const { return output_device_; }
TrackInfo Player::GetTrackInfo() const { return track_; }
int64_t Player::GetPositionSamples() const { return position_samples_.load(); }
...
void Player::DecodeThread() {
    ...
    while (!stop_decode_.load()) {
        std::filesystem::path request_path;
        {
            ...
            request_cv_.wait_for(lock, std::chrono::milliseconds(5), [this] {
                return request_pending_ || stop_decode_.load();
            });
        ...
        if (available > ring_capacity_ * 3 / 4) {
            std::this_thread::sleep_for(std::chrono::milliseconds(FrameDurationMs(frames, sample_rate_)));
            continue;
        }
...
bool Player::InitAudio(int sample_rate, int channels) {
    ...
    if (device_ == 0) {
        device_ = SDL_OpenAudioDevice(dev_ptr, 0, &want, &got,
    ...
    if (got.freq != want.freq || got.channels != want.channels) {
        Logger::Warn("Audio device negotiated different format: " +
            std::to_string(got.freq) + "Hz " + std::to_string(got.channels) + "ch");
    }
